...
# symoditi autocomplete (machine-generated by the installer; harmless if absent)
SYMODITI_AC_ZSH_SETUP_PATH="$HOME/Library/Caches/symoditi/autocomplete/zsh_setup" && test -f "$SYMODITI_AC_ZSH_SETUP_PATH" && source "$SYMODITI_AC_ZSH_SETUP_PATH"

# Claude Code: per-account isolation via CLAUDE_CONFIG_DIR
# ~/.claude         → agent@vasily.dev (default, work, AgentOS infra)
# ~/.claude-backup  → krylov@vasily.dev (backup Max plan)
alias claude-backup='CLAUDE_CONFIG_DIR=$HOME/.claude-backup claude'
...
# GAM (Google Workspace admin), macOS install path
if [[ "$OSTYPE" == darwin* ]]; then
